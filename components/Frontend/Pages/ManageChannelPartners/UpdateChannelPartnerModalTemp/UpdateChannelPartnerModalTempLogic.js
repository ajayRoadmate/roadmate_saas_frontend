...
export default function UpdateChannelPartnerModalTempLogic(updateModalStates, updateModalTasks, tableTasks){

    const{fromStates, formTasks} = FormModule();

    var INITIAL_FORM_STATE = {
        status: 'loading',
        id:'updateChannelPartnerFormId',
        submitButtonValue: 'Update',
        submitEndpoint: 'admin_updateChannelPartner', 
        inputs: [
            {name: 'channel_partner_name', id: 'channel_partner_name', label: 'Channel Partner Name', type: 'text', pattern: '^.{5,50}$', isRequired: true, invalidFeedback: 'Required, 5-50 characters long.'},
            {name: 'address', id: 'address', label: 'Address', type: 'textArea', pattern: '^.{5,50}$', isRequired: true, invalidFeedback: 'Required, 5-50 characters long.'},
            ...
            {name: 'gst_number', id: 'gst_number', label: 'GST Number', type: 'text', pattern: '^.{0,15}$', isRequired: false, invalidFeedback: 'Optional, 0-15 characters long.'},
    ...
    async function initializeForm(){
        ...
        try{
            ...
            var endpoint = 'admin_fetchChannelPartnerUpdateFormData';

            var updateFormData = await updateModalTasks.getUpdateFormData(itemKey, itemValue, endpoint);

            var distributorSelectOptions = {selectInputName: 'distributor_id', endpoint: 'fetchDistributerFilterData'};
            await formTasks.initializeSelect(distributorSelectOptions);
            ...
            var populateFormDataOptions = {endpoint:'admin_fetchChannelPartnerUpdateFormData', params:{item_key: itemKey, item_value: itemValue}, initialFormState: INITIAL_FORM_STATE};
